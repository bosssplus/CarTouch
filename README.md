...
| 📊 **مانیتورینگ** | سرعت، RPM، دمای موتور، ولتاژ باطری، سطح سوخت + خواندن کدهای خطا (DTC) |
| 🎓 **یادگیری (v2.0)** | ضبط فرمان از دکمه‌های فیزیکی واقعی خودرو، یا ورود دستی CAN ID/بایت |
...
| 🛡️ **ایمنی** | حالت Listen-Only، محدودیت نرخ فرمان، اجبار تغییر رمز پیش‌فرض، توکن نشست WebSocket |
| 🔋 **انرژی** | Auto Sleep بعد از ۱۰ دقیقه بی‌فعالیتی + بیدار شدن با پیام CAN |
| 🌗 **نمایش** | حالت شب/روز خودکار یا دستی |
...
| موضوع | وضعیت |
|---|---|
| خواندن OBD-II استاندارد (RPM، سرعت، دما، …) | ✅ آماده |
...
| فایل‌های DBC | ۵۷ فایل همراه پروژه است، ولی فعلاً فقط **۳ مدل** (Toyota Prius 2010، Toyota 2017 Ref PT، BMW E9x/E8x) + حالت عمومی OBD-II در لیست انتخاب وصل شده‌اند |
| پارسر DBC | فقط سیگنال‌های Intel (`@1`)؛ Motorola (`@0`) هنوز پشتیبانی نمی‌شود. سقف ۵۰ پیام برای هر فایل (فایل‌های بزرگ‌تر بریده می‌شوند) |
| Listen-Only | پیش‌فرض **خاموش** است. تغییرش در زمان اجرا فقط فیلتر نرم‌افزاری است؛ درایور TWAI فقط هنگام بوت با حالت ذخیره‌شده نصب می‌شود |
| کالیبراسیون تاچ | مقادیر فعلی نمونه (placeholder) است و باید روی نمایشگر خودتان تنظیم شود |
| HTTPS | ندارد — ترافیک وب رمزنگاری نمی‌شود |

> کارکرد نهایی روی خودروی واقعی فقط با تست خودتان روی سخت‌افزار قابل تأیید است.
> فهرست کامل مشکلات شناخته‌شده: [`CHANGELOG.md`](./CHANGELOG.md#-مشکلات-شناختهشده-و-کارهای-باقیمانده)
...
> معماری، الگوریتم و قوانین ایمنی: [`CarTouch_V2_SPEC.md`](./CarTouch_V2_SPEC.md)
...
```bash
git clone https://github.com/<USERNAME>/CarTouch_V2.git
cd CarTouch_V2
...
> [!NOTE]
> پوشه‌ی `data/` حدود **۳٫۴ مگابایت** است. اگر `uploadfs` خطای «حجم بیش‌ازحد» داد، DBC های غیرضروری را از `data/dbc/` حذف کنید (فقط ۳ فایل وایرشده در `vehicle_db.cpp` لازم‌اند) یا یک جدول پارتیشن با SPIFFS بزرگ‌تر تعریف کنید.
...
|---|---|---|---|
| TFT_eSPI | ≥ 2.5.43 | BSD | درایور نمایشگر |
| lvgl | 8.4.x | MIT | رابط گرافیکی |
| ESPAsyncWebServer | ≥ 3.7.0 | LGPL | وب سرور Async |
...
2. در مرورگر آدرسی که در Serial Monitor چاپ می‌شود را باز کنید (معمولاً `192.168.4.1`).
3. با نام کاربری `admin` و رمز موقت `cartouch` وارد شوید.
4. **همین اول رمز را عوض کنید** (حداقل ۸ کاراکتر، متفاوت از رمز پیش‌فرض) — از تب تنظیمات وب یا از صفحه‌ی لمسی. تا تغییر ندهید، دستگاه هر بار هشدار نشان می‌دهد.
...
├── CarTouch_V2_SPEC.md           # مشخصات فنی کامل v2.0 (مرجع توسعه)
...
│   ├── vehicle_db.cpp/.h         # پارسر DBC
│   ├── tft_ui.cpp/.h             # رابط لمسی LVGL — ۴ تب
│   ├── webserver.cpp/.h          # وب سرور + WebSocket
...
|---|---|
| [`CarTouch_V2_SPEC.md`](./CarTouch_V2_SPEC.md) | معماری Learn Mode، الگوریتم، قوانین ایمنی، ترتیب توسعه — مرجع کامل برای ادامه‌ی توسعه |
